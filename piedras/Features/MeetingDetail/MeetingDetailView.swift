...
struct MeetingDetailView: View {
    ...
    private func canGenerateAudioSummary(for meeting: Meeting) -> Bool {
        if let audioRemotePath = meeting.audioRemotePath?.trimmingCharacters(in: .whitespacesAndNewlines),
           !audioRemotePath.isEmpty {
            return true
        }

        if let audioLocalPath = meeting.audioLocalPath?.trimmingCharacters(in: .whitespacesAndNewlines),
           !audioLocalPath.isEmpty,
           FileManager.default.fileExists(atPath: audioLocalPath) {
            return true
        }
