...
## 项目结构与模块组织
`CocoInterview/` 是 SwiftUI iOS 客户端。应用启动与容器放在 `CocoInterview/App`，功能页面放在 `CocoInterview/Features`，状态管理放在 `CocoInterview/Stores`，通用服务放在 `CocoInterview/Services`，数据模型与持久化放在 `CocoInterview/Data`，共享组件与工具放在 `CocoInterview/Shared`。

`CocoInterviewTests/` 存放 XCTest 单元测试，覆盖仓储、Store、Service 和部分界面展示逻辑；`CocoInterviewUITests/` 覆盖录音、导航等关键流程。服务端代码在 `cloud/api/`（Next.js + Prisma）与 `cloud/asr-proxy/`（旧版 WebSocket 代理）。说明文档放在 `docs/`，辅助脚本放在 `scripts/`。
...
新增代码应尽量放入现有最小职责模块。页面专属 UI 优先放在 `CocoInterview/Features/<Area>/`；只有在跨页面复用时才提升到 `Shared/`。
...
每次提交只包含一个清晰的逻辑变更。不要提交构建产物、临时截图、`tmp-ui-test-*` 导出文件或个人 Xcode 配置。PR 需要说明用户可见影响，标明涉及目录如 `CocoInterview/` 或 `cloud/api/`，关联 Issue，并为 UI 改动附上截图。
...
## 配置说明
默认以生产后端链路为准。若需切换后端，请通过 `COCO_INTERVIEW_BACKEND_BASE_URL` 等构建配置覆盖，不要在应用代码中硬编码本地地址。
