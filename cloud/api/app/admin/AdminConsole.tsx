...
export function AdminConsole({
  ...
  return (
    <section id="account-admin" className="admin-section">
      <section className="glass-card">
        <div className="admin-topbar">
          <div>
            ...
            <h2 className="admin-title">邀请码与账号管理</h2>
            <p className="muted admin-subtitle">
              在同一个页面里完成邀请码、测试账号、legacy 数据接管和密码重置。
            </p>
          </div>
      ...
      {!session.configured ? (
        <section className="glass-card">
          <h3 className="section-title">先补管理员密钥</h3>
          <p className="muted">
...
vercel env add ADMIN_API_SECRET development`}
      ...
      {session.configured && !session.authenticated ? (
        <section className="glass-card admin-form-card">
          <h3 className="section-title">管理员登录</h3>
        ...
        <>
          <section className="card-grid">
            <article className="glass-card">
              ...
              <h3 className="section-number">{dashboard.schema.ready ? 'Ready' : 'Pending'}</h3>
              <p className="muted">
            ...
            <article className="glass-card">
              ...
              <h3 className="section-number">{dashboard.users.length}</h3>
              <p className="muted">已创建并可管理的账号数量</p>
            </article>

            <article className="glass-card">
              <p className="muted small-label">Legacy 工作区</p>
              <h3 className="section-number">{dashboard.legacyWorkspaces.length}</h3>
              <p className="muted">尚未挂到账号下的历史数据空间</p>
            </article>

            <article className="glass-card">
              <p className="muted small-label">邀请码</p>
              <h3 className="section-number">{dashboard.inviteCodes.length}</h3>
          ...
          {!dashboard.schema.ready ? (
            <section className="glass-card">
              <h3 className="section-title">数据库还没落库</h3>
              <p className="muted">
...
npx prisma db push`}
            ...
            <section className="admin-split-grid">
              <article className="glass-card admin-form-card">
                <h3 className="section-title">创建托管账号</h3>
                <p className="muted">
              ...
              <article className="glass-card admin-form-card">
                <h3 className="section-title">生成邀请码</h3>
              ...
              <article className="glass-card admin-form-card">
                <h3 className="section-title">把 legacy 数据交给已有账号</h3>
                <p className="muted">
          ...
          <section className="glass-card">
            <h3 className="section-title">Legacy 工作区现状</h3>
          ...
          <section className="glass-card">
            <h3 className="section-title">账号列表</h3>
            <div className="admin-table-wrap">
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    dashboard.users.map((user) => (
                      <tr key={user.id}>
                        <td>
                          ...
                          <div className="table-secondary">{user.displayName || '未设置昵称'}</div>
                        </td>
                        <td>
                          {user.workspace ? (
                            <>
          ...
          <section className="glass-card">
            <h3 className="section-title">邀请码列表</h3>
            <div className="admin-table-wrap">
              <table className="admin-table">
                <thead>
                  <tr>
