# 椰子面试 iOS 核心 MVP 重构计划

> 2026-03-18 更新：仓库边界已改为单主仓模式。`coco-interview-ios` 同时承载 iOS 客户端、`cloud/api` 和 `cloud/asr-proxy`，不再把 `ai_notepad` 作为长期主仓。

## 当前实现基线

- iOS 默认连接固定云后端 `https://api.coco-interview.example.com`
- LLM 固定走 AiHubMix 的 OpenAI-compatible 接口
- ASR 固定走豆包流式识别，经后端签发 session，再通过同域名统一入口转发
- 设置页已从开发态后端配置改为产品态服务状态页
- `workspace` 仅作为后端兼容层在内部保留，不在 iOS UI 暴露

## 1. 项目目标

椰子面试 现有 Web 端已经具备录音、转写、AI 总结、工作区、知识库、面试追踪、MCP 等较完整能力，但 iOS 首发版本不追求功能对齐，而是聚焦一个可以快速上线、真实可用、容易持续迭代的录音笔记核心产品。

本次 iOS MVP 的目标是：

- 以原生 `Swift + SwiftUI` 构建一个独立的 iOS App。
- 以本地录音、本地存储、本地可编辑笔记为主，保证离线可用。
- 在同一仓库内承载 iOS 依赖的 Next.js 云端 API，避免再拆多个主仓维护。
- 复用现有豆包实时 ASR 与现有 LLM 后端能力。
...
- 首页采用 Granola 风格会议 Feed：
  - 大标题
  - 按日期分组的会议卡片
  - 底部 `Ask anything + Record` dock
- 详情页保留单页文档感：
  - 自定义头部
  - `Smart Notes / Transcript` 双态切换
  - 笔记与 AI 总结优先展示
- 搜索、设置、全局 AI 统一通过 sheet 进入。
- 所有复杂概念从 iOS UI 中移除，只在内部保留隐藏 workspace 兼容后端。
...
- `cloud/api`：Next.js API + Prisma，也是当前推荐的 API + ASR 统一入口
- `cloud/asr-proxy`：旧的独立豆包 WebSocket 代理，仅用于兼容和单独调试
