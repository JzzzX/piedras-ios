...
> 2026-03-18 更新：仓库边界已改为单主仓模式。`piedras-ios` 同时承载 iOS 客户端、`cloud/api` 和 `cloud/asr-proxy`，不再把 `ai_notepad` 作为长期主仓。

## 当前实现基线

- iOS 默认连接固定云后端 `https://piedras.preview.aliyun-zeabur.cn`
