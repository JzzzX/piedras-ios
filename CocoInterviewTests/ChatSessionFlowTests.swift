...
struct ChatSessionFlowTests {
    ...
    @MainActor
    @Test
    func uiTestBackendProtocolInterceptsLoopbackRequestsWithoutLaunchArgument() throws {
        let request = try #require(URLRequest(url: URL(string: "http://127.0.0.1:3000/api/chat")!))

        #expect(UITestBackendURLProtocol.canInit(with: request))
    }

    @MainActor
    @Test
    func appContainerUITestBackendSupportsMeetingChatWithoutLoopbackServer() async throws {
        let container = try ModelContainerFactory.makeContainer(inMemory: true)
        let settingsStore = makeSettingsStore()
        settingsStore.workspaceBootstrapState = .success
        settingsStore.applyDebugBackendOverride(SettingsStore.simulatorLoopbackBaseURLString)
        let apiClient = APIClient(
            settingsStore: settingsStore,
            session: UITestBackendURLProtocol.makeSession()
        )
        let meetingRepository = MeetingRepository(modelContext: container.mainContext)
        let chatRepository = ChatSessionRepository(modelContext: container.mainContext)
        let meetingSyncService = MeetingSyncService(
        ...
        let didSend = await meetingStore.sendChatMessage(question: "帮我总结", for: meeting.id)

        #expect(didSend)
        try await waitUntil {
            meetingStore.chatMessages(for: meeting.id).map(\.content) == ["帮我总结", "这是来自测试后端的回答。"]
        }
    }
    ...
    private func makeSettingsStore() -> SettingsStore {
        let suiteName = "cocointerview.tests.chat.\(UUID().uuidString)"
