...
final class AppContainer {
    ...
    init(inMemory: Bool = false) {
        ...
        let shouldUseUITestBackendStub = processArguments.contains("UITEST_USE_SIMULATOR_BACKEND")
        let settingsDefaults: UserDefaults
        if shouldUseIsolatedDefaults,
           let ephemeralDefaults = UserDefaults(suiteName: "cocointerview.ui-tests.in-memory") {
            ephemeralDefaults.removePersistentDomain(forName: "cocointerview.ui-tests.in-memory")
        ...
        apiClient = APIClient(
            ...
            session: shouldUseUITestBackendStub ? UITestBackendURLProtocol.makeSession() : .shared
        )
        authStore = AuthStore(
            apiClient: apiClient,
        ...
        if inMemory {
            ...
            authStore.currentUser = .init(id: "preview-user", email: "preview@coco-interview.local")
