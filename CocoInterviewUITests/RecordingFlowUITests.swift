...
final class RecordingFlowUITests: XCTestCase {
    ...
    func testHomeDockUsesTwoSlotLayoutAndBrandOnlyTitle() throws {
        ...
        XCTAssertTrue(app.staticTexts["椰子面试"].waitForExistence(timeout: 5), "首页品牌标题未出现。")
        XCTAssertFalse(app.staticTexts["椰子面试 笔记"].exists, "首页标题应简化为 椰子面试。")
    ...
    func testHomeFolderButtonOpensDrawerAndHidesBottomDock() throws {
        ...
        XCTAssertFalse(app.buttons["NewRecordingButton"].isHittable, "打开文件夹抽屉时底部录音入口不应继续可交互。")
        XCTAssertFalse(app.buttons["HomeGlobalChatLauncher"].isHittable, "打开文件夹抽屉时底部对话入口不应继续可交互。")

        app.buttons["FolderDrawerCloseButton"].tap()

        XCTAssertFalse(newFolderButton.isHittable, "关闭后文件夹抽屉不应继续可交互。")
    ...
    func testMeetingRowSupportsSwipeToDelete() throws {
        let app = launchApp()
        let seededTitle = "椰子面试 iOS MVP Kickoff"
    ...
    func testMeetingDetailMenuShowsImmersiveActions() throws {
        ...
        XCTAssertFalse(app.staticTexts["椰子面试 AI"].exists, "详情页不应再展示品牌标题。")
